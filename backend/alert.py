...
def send_emergency_alerts(
    ...
    try:
        ...
        cursor.execute("SELECT doctor_id FROM patients WHERE patient_id = ? OR phone = ?", (patient_id, patient_phone))
        pat_row = cursor.fetchone()
        if pat_row and pat_row["doctor_id"]:
            doctor_id = pat_row["doctor_id"]
            # Fetch doctor's live details
            cursor.execute("SELECT name, phone FROM doctors WHERE doctor_id = ?", (doctor_id,))
            doc_row = cursor.fetchone()
            if doc_row:
                doctor_name = doc_row["name"]
                doctor_phone = doc_row["phone"]
                print(f"[Alert] Using live doctor details from DB: name={doctor_name}, phone={doctor_phone}")
